...
func ContactFinder(op []int, words []string) []int {
	res := make([]int, 0)
	trie := NewContactTrieNode()
	for i := 0; i < len(op); i++ {
		if op[i] == 0 {
			// insert
			trie.Insert(words[i])
			...
			// search
			res = append(res, trie.Search(words[i]))
		}
	}
...
type ContactTrieNode struct {
	hash map[byte]*ContactTrieNode
	end  bool
	freq int
}

func NewContactTrieNode() ContactTrieNode {
	return ContactTrieNode{hash: map[byte]*ContactTrieNode{}}
}

func (this *ContactTrieNode) Insert(word string) {
	curr := this

	for i := 0; i < len(word); i++ {
		if _, ok := curr.hash[word[i]]; !ok {
			x := NewContactTrieNode()
			curr.hash[word[i]] = &x
		}
...
func (this *ContactTrieNode) Search(word string) int {
	curr := this
	count := 0
	for i := 0; i < len(word); i++ {
		curr = curr.hash[word[i]]
